...
/*
 *  oPAPI_register -- registers and starts hardware performance monitoring counters.
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    {
        char error[128];
        sprintf(error, "oPAPI_register takes from 1 to %d "
    ...
    for (i = 0; i < nrhs; ++i)
    {
        if (!mxIsChar(prhs[i]))
        {
            mexErrMsgTxt("oPAPI_register requires a string with hardware counter name.");
        }
    ...
    {
        char error[128];
        sprintf(error, "oPAPI_register couldn't start counters. Error message: %s", PAPI_strerror(result));
        mexErrMsgTxt(error);
    }
    /* Register number of running counters */
    oPAPI_setGlobalStatus(nrhs);
}
